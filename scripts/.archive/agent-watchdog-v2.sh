...
PROCESS_REGISTRY_FILE = "/Users/fonsecabc/.openclaw/tasks/process-registry.json"
...
def send_failure_alert(state):
    ...
    msg = (
        f"<@U0AJU1XN3AT> :rotating_light: *{count} consecutive agent failures detected*\n"
    ...
    # Post alert to #replicants channel
    ...
    try:
        r = subprocess.run(
            ["curl", "-s", "-X", "POST", "https://slack.com/api/chat.postMessage",
             ...
             "-d", json.dumps({"channel": REPLICANTS_CHANNEL, "text": msg, "mrkdwn": True})],
            capture_output=True, text=True, timeout=10
        )
        resp = json.loads(r.stdout)
        if resp.get("ok"):
            print(f"ALERT SENT: {count} consecutive failures → #replicants")
        else:
            print(f"ALERT FAILED: {r.stdout[:200]}")
...
def has_registered_process(task_id):
    """Check if this task has a running process in the process registry.
    If so, the agent exiting is expected (it registered a long-running process and exited cleanly)."""
    try:
        with open(PROCESS_REGISTRY_FILE) as f:
            procs = json.load(f).get("processes", {})
        for proc_id, proc in procs.items():
            if proc.get("taskId") == task_id and proc.get("status") == "running":
                return True
    except Exception:
        pass
    return False
...
for task_id, a in agents.items():
    ...
    if not alive:
        # Check if agent registered a long-running process before exiting.
        # If so, this is an EXPECTED exit — the process manager handles completion.
        if has_registered_process(task_id):
            print(f"PROCESS_HANDOFF {task_id}: Agent exited after registering process. Process manager will handle callback.")
            removals.append(task_id)
            # Don't count as failure or completion — process manager owns the lifecycle now
            continue
...
metrics = {
    ...
    "total_agents": total,
    "failed_agents": total_failed + total_timeout,
    "window_days": 7,
    "summary": {
        "total_agents": total,
...
if alerts and SLACK_BOT_TOKEN:
    ...
    for a in alerts:
        if a["type"] == "low_success_rate" and should_send_alert("low_success_rate"):
            try:
                msg = (f"<@U0AJU1XN3AT> :warning: *Agent health alert*\n"
                ...
                print(f"  Health alert sent to #replicants")
            except Exception as e:
