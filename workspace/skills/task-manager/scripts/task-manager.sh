...
STATE_FILE="$OC_HOME/tasks/state.json"
...
cmd_status() {
    ...
    if [ -d "$OC_HOME/workflows" ]; then
        workflows=$(find $OC_HOME/workflows -name "*-state.json" 2>/dev/null | wc -l)
        if [ "$workflows" -gt 0 ]; then
            for state_file in $OC_HOME/workflows/*-state.json; do
