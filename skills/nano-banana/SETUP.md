...
**File:** `/root/.openclaw/workspace/config/mcporter.json`
...
```bash
cd /root/.openclaw/workspace/skills/nano-banana
./test-demo.sh
```
...
cat /root/.openclaw/workspace/config/mcporter.json | grep nano-banana
...
⚠️ **Config Update:** Replace `YOUR_GEMINI_API_KEY_HERE` in `/root/.openclaw/workspace/config/mcporter.json`
...
2. **Update mcporter config** at `/root/.openclaw/workspace/config/mcporter.json`
